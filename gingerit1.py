...
class gt(object):
    ...
    def parse(self, text, verify=True):
        ...
        request = session.get(
            self.url,
            params={"lang": self.lang,"apiKey": self.api_key,"clientVersion": self.api_version,"text": text,},verify=verify,
            )
        data = request.json()
        return data
    ...
    def _change_char(original_text, from_position, to_position, change_with):
        return "{}{}{}".format(original_text[:from_position], change_with, original_text[to_position + 1 :])
    def _process_data(self, text, data):
        result = text
        return result 
    def _process(self,data,result,n,text):
        corrections=[]
        for suggestion in reversed(data["Corrections"]):
            ...
            if suggestion["Suggestions"]:
                ...
                corrections.append({"start": start,"text": text[start : end + 1],"correct": suggest.get("Text", None),
